...
export function PremiumCard({ children, className = "", ...rest }: PremiumCardProps) {
  return (
    <article
      {...rest}
      className={`rounded-2xl border border-white/10 bg-gradient-to-b from-[#1a1a1f] to-[#141419] p-4 shadow-[0_10px_35px_rgba(0,0,0,0.45)] ${className}`}
    >
      {children}
    </article>
  );
...
export function PrimaryButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`rounded-xl px-3 py-2 text-xs font-medium transition active:scale-[0.99] disabled:cursor-not-allowed disabled:active:scale-100 ${className}`}
    >
      {children}
    </button>
  );
...
export function TextInput({
  ...
  return (
    <div>
      <label htmlFor={id} className="text-[11px] uppercase tracking-wide text-zinc-400">
        {label}
      </label>
      <input
        id={id}
        ...
        className="mt-1 w-full rounded-xl border border-white/10 bg-white/5 px-3 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-500 transition focus:border-[#c9a35c]/70 focus:outline-none disabled:cursor-not-allowed disabled:opacity-60"
      />
    </div>
  );
}
...
export function SongCard({
  ...
  return (
    <div
      className={
        isMustPlay
          ? "rounded-xl border border-[#c9a35c]/30 bg-gradient-to-r from-[#c9a35c]/12 to-white/[0.02] p-3 shadow-[0_6px_18px_rgba(0,0,0,0.35)]"
          : "rounded-xl border border-[#7a5c5c]/35 bg-gradient-to-r from-[#7a5c5c]/25 to-[#1d1d22] p-3 shadow-[0_6px_18px_rgba(0,0,0,0.35)]"
      }
    >
...
export function BottomNav({ items, activeScreen, onSelect }: BottomNavProps) {
  return (
    <nav className="fixed inset-x-0 bottom-0 mx-auto w-full max-w-md border-t border-[#c9a35c]/20 bg-[#0a0a0c]/95 px-4 pb-5 pt-3 backdrop-blur">
      <ul
          ...
          <li key={item}>
            <PrimaryButton
              ...
              className={`w-full px-2 py-2 ${
                activeScreen === item
                  ? "bg-[#c9a35c]/20 text-[#f5e6c8] shadow-[0_6px_16px_rgba(0,0,0,0.35)]"
